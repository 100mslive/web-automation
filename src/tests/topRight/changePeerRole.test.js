...
test(`Change peer Role`, async ({ context }) => {
  ...
  for (let i = 0; i < pages[0].topRight.role_list.length; i++) {
    ...
    await pages[0].click(
      pages[0].topRight.participant_list,
      pages[0].topRight.participant_setting.replace("?", pages[1].localName),
      pages[0].topRight.dialog_select_change_role_to
    );
    ...
    await pages[1].click(
      pages[1].topRight.dialog_accept,
      pages[1].topRight.participant_list
    );
    await pages[1].assertVisible(
      pages[1].topRight.participant_role_heading.replace(
        "?",
        pages[1].topRight.role_list[i]
      )
    );
    ...
    await pages[0].click(
      pages[0].topRight.participant_list,
      pages[0].topRight.setting_role_peer0.replace("?", 2),
      pages[0].topRight.dialog_select_change_role_to
    );
