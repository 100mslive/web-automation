...
test(`Verify greeting tile for first participant`, async ({
  page: nativePage,
}) => {
...
test(`Verify Join peers`, async ({ context }) => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (let j = 0; j < 5; j++) {
      const participantName = pages[i].topRight.participant_number.replace(
        "?",
        pages[j].localName
      );
...
test(`Verify network on tile and peerlist`, async ({ context }) => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    await pages[i].assertVisible(
      pages[i].tiles.network_ontile.replace("?", pages[i].localName)
    );
  }
  await pages[0].endRoom();
  await context.close();
